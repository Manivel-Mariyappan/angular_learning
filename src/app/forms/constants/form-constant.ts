export const forms = [
    {
        name: 'Simple Reactive Form',
        path: 'simple-reactive-form'
    },
    {
        name: 'Reactive form with form Group',
        path: 'reactive-form',
    },
    {
        name: 'Reactive form builder demo',
        path: 'reactive-formbuilder'
    },
    {
        name: 'Template Driven',
        path: 'template-driven-form'
    }
]

export const genderOptions = [
    { name: 'Male', value: 'male' },
    { name: 'Female', value: 'female' },
    { name: 'Other', value: 'other' }
];

export const states = [
    { id: 1, name: 'Andhra Pradesh' },
    { id: 2, name: 'Arunachal Pradesh' },
    { id: 3, name: 'Assam' },
    { id: 4, name: 'Bihar' },
    { id: 5, name: 'Chhattisgarh' },
    { id: 6, name: 'Goa' },
    { id: 7, name: 'Gujarat' },
    { id: 8, name: 'Haryana' },
    { id: 9, name: 'Himachal Pradesh' },
    { id: 10, name: 'Jharkhand' },
    { id: 11, name: 'Karnataka' },
    { id: 12, name: 'Kerala' },
    { id: 13, name: 'Madhya Pradesh' },
    { id: 14, name: 'Maharashtra' },
    { id: 15, name: 'Manipur' },
    { id: 16, name: 'Meghalaya' },
    { id: 17, name: 'Mizoram' },
    { id: 18, name: 'Nagaland' },
    { id: 19, name: 'Odisha' },
    { id: 20, name: 'Punjab' },
    { id: 21, name: 'Rajasthan' },
    { id: 22, name: 'Sikkim' },
    { id: 23, name: 'Tamil Nadu' },
    { id: 24, name: 'Telangana' },
    { id: 25, name: 'Tripura' },
    { id: 26, name: 'Uttar Pradesh' },
    { id: 27, name: 'Uttarakhand' },
    { id: 28, name: 'West Bengal' },
    { id: 29, name: 'Andaman and Nicobar Islands' },
    { id: 30, name: 'Chandigarh' },
    { id: 31, name: 'Dadra and Nagar Haveli and Daman and Diu' },
    { id: 32, name: 'Lakshadweep' },
    { id: 33, name: 'Delhi' },
    { id: 34, name: 'Puducherry' }
];


export const sampleDbValue = {
    firstName: "John",
    lastName: "Doe",
    gender: "male",
    email: "john.doe@example.com",
    mobile: "+91-9876543210",
    address: "123, Main Street, Example City",
    state: "4",
    city: "Mumbai",
    zip: "400001",
    hobbies: "Reading, Traveling, Sports",
    skills: [
        {
            name: "JavaScript",
            description: "Expert in JavaScript and related frameworks like Angular, React"
        },
        {
            name: "CSS",
            description: "Proficient in CSS and responsive design"
        }
    ]
}
